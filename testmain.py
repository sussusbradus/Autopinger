...
# --- BANLIST CHECK ---
# The old @bot.check only covered prefix commands. For slash commands the
# equivalent hook lives on the command tree, so we subclass it.
...
# swallow the CheckFailure raised when the banlist check returns False,
...
# joke
@bot.tree.command(name="fuckyou", description="Rude")
async def fuckyou(interaction: discord.Interaction):
    await interaction.response.send_message("yea im pissed now")
